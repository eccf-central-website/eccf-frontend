 ...
 * Dynamically connected to Sanity CMS Studio with live editing, pinning, and category filtering.
 */
...
function parsePublishDate(dateStr?: string) {
  if (!dateStr) return { day: '01', month: 'JAN', year: '2025', full: 'July 20, 2025' }
  const d = new Date(dateStr)
  if (isNaN(d.getTime())) return { day: '01', month: 'JAN', year: '2025', full: dateStr }
  const months = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC']
  const fullMonths = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
  const day = String(d.getDate()).padStart(2, '0')
  const month = months[d.getMonth()]
  const year = String(d.getFullYear())
  const full = `${fullMonths[d.getMonth()]} ${d.getDate()}, ${year}`
  return { day, month, year, full }
}

const fallbackAnnouncements: ParsedAnnouncement[] = [
  {
    _id: 'announcement-1',
...
export default function AnnouncementsPage() {
  const [announcements, setAnnouncements] = useState<ParsedAnnouncement[]>(fallbackAnnouncements)
  ...
  useEffect(() => {
    sanityClient
        ...
        if (data && data.length > 0) {
          const parsed = data.map((item) => {
            ...
            return {
              ...
              category: (item.category || 'WEEKLY SERVICES').toUpperCase(),
              isPinned: Boolean(item.isPinned),
              day: dateObj.day,
              month: dateObj.month,
              year: dateObj.year,
              publishDate: dateObj.full,
              time: item.time || 'Service Time',
      ...
      .catch((err) => {
        console.warn('Could not fetch live Sanity announcements, using defaults:', err)
      })
  }, [])

  // Build dynamic category list from existing announcements
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                ...
                <div className="md:col-span-10 space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {item.time && item.time !== 'Service Time' && (
